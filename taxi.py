...
class Taxi:
    ...
    def __init__(self, world, taxi_num, idle_loss=256, max_wait=50, on_duty_time=0, off_duty_time=0, service_area=None,
        ...
        # -1 is have not received a fare, 0 is currently doing first fare, anything else is revenue generated.
        self._revenue = -1
        ...
        # Financial Status
        # 0 is desperate, 1 is normal, 2 is relaxed
        self._financialStatus = 0

        # Average Revenue of all the active Taxis
    ...
    def pickUp(self):
        print("Taxi " + str(self.number) + " has picked up customer")
        print("Taxi " + str(self.number) + " has pre-drop off revenue number of " + str(self._revenue))
        if self._revenue == -1:
            print("Taxi " + str(self.number) + " first time fare detected, set to 0.")
            self._revenue = 0

    def dropOff(self, price):
        print("Taxi " + str(self.number) + " has dropped off customer")
        self._revenue = self._revenue + price
        print("Taxi " + str(self.number) + " has post-drop off a revenue number of " + str(self._revenue))

    def cancelledFare(self):
        if self._revenue == -1:
            self._revenue = -1
            print("Taxi "+ str(self.number) + "'s First Fare was cancelled, revenue reset to: -1")
        elif self._revenue != 0 and self._revenue != -1:
            print("Taxi " + str(self.number) + "'s Fare was cancelled, revenue is still: " + str(self._revenue))
        else:
            print("Taxi " + str(self.number) + "'s Fare was cancelled, despite customer being picked up.")

    def revenue(self):
        return self._revenue

    def financialStatus(self):
        return self._financialStatus
    ...
    def comeOnDuty(self, time=0):
        ...
        if self._offDutyTime == 0 or (time >= self._onDutyTime and time < self._offDutyTime):
            ...
            print("Taxi " + str(self.number) + " has: " + str(self._account))
    ...
    def clockTick(self, world):
        # check Taxi account balance * decay rate against 20th Percentile Cancel Time:
        # unnecessary, but its so can experiment with different decay rates.
        if self._percentileCancelTime > 0 and self.isOnDuty:
            timeRemaining = self._account * 1
            timeAverage = timeRemaining/self._percentileCancelTime
            if 3.0 <= timeAverage <= 6.0:
                self._financialStatus = 1
                print("Taxi " + str(self.number) + " is alright, account balance is: " + str(timeRemaining) + " / " + str(self._percentileCancelTime))
            elif 6.0 < timeAverage:
                self._financialStatus = 2
                print("Taxi " + str(self.number) + " is balling, account balance is: " + str(timeRemaining) + " / " + str(self._percentileCancelTime))
            else:
                self._financialStatus = 0
                print("Taxi " + str(self.number) + " is desperate, account balance is: " + str(timeRemaining) + " / " + str(self._percentileCancelTime))
    ...
    # Added:
    # 20th Fare Cancel Time args to new bidding opportunity.
    def recvMsg(self, msg, **args):
        ...
        if msg == self.FARE_ADVICE:
            ...
            self._percentileCancelTime = args['percentileCancelTime']
            self._avgTaxisRevenue = args['avgTaxisRevenue']
            if self.onDuty:
                print("Taxi " + str(self.number) + " received percentileCancelTime " + str(self._percentileCancelTime))
                print("Taxi " + str(self.number) + " received avgTaxisRevenue " + str(self._avgTaxisRevenue) + ", personal is: " + str(self._revenue))
        ...
        # the dispatcher has approved our bid: mark the fare as ours
            ...
            print("Taxi " + str(self.number) + " received approval from Dispatcher")
            for fare in self._availableFares.items():
        ...
        # we just dropped off a fare and received payment, add it to the account
            ...
            print("Taxi " + str(self.number) + " has completed a fare. \n" + "Amount now: " + str(self._account) + ", revenue score is: " + str(self._revenue))
            return
        # a fare cancelled before being collected, remove it from the list
        elif msg == self.FARE_CANCEL:
            print("Fare cancelled for taxi: " + str(self.number))
            self.cancelledFare()
            for fare in self._availableFares.items():
    ...
    def _planPath(self, startNode, destination):
        if startNode not in self._map:
            return None
        if startNode == destination:
            return [startNode]
        backTrace = {}
        # backTrace stores visited nodes, and the parent of the node
        # origin is stored in backTrace as origins parent is the origin <- sentinel value
        backTrace.update([(startNode, startNode)])
        self.queue = [startNode]
        while len(self.queue) > 0:
            curNode = self.queue.pop(0)
            # expansion, adjacent unexplored nodes
            expansion = dict([(node, curNode) for node in self._map[curNode].keys() if node not in backTrace])
            # check if destination is adjacent to the current node
            if destination in expansion:
                path = [destination]
                parent = curNode
                # go through the parents till sentinel value reached, then reverse path to produce the correct path
                while parent in backTrace and parent != startNode:
                ...
                path.reverse()
                return path
            # if no adjacent node is the destination, then add the adjacent unexplored nodes to the queue
            else:
                self.queue.extend(expansion.keys())
                backTrace.update(expansion)
        return None

    # TODO
    # this function decides whether to offer a bid for a fare. In general, you can consider your current position, time,
    # financial state, the collection and drop-off points, the time the fare called - or indeed any other variable that
    ...
    # Added:
    # 20th Percentile Cancel Time Check
    # if pay > journey to pick-up and destination
    ...
    def _bidOnFare(self, time, origin, destination, price):
        ...
        FareExpiryLenience = 0

        # Check Financial Situation
        if self._financialStatus == 1:
            FareExpiryLenience = 0
        elif self._financialStatus == 2:
            FareExpiryLenience = -5
        # New Property, BelowCancelThreshold
        # New Property, EstimatedExpiryTimeOfFare
        # 20th Percentile Cancel Time - Elapsed time > TimeToOrigin
        # Only does this if not 'Desperate'
        if self._financialStatus != 0:
            ElapsedTime = self._world.simTime - time
            EstimatedExpiryTimeOfFare = self._percentileCancelTime - ElapsedTime
            print("Elapsed Time since fare was opened: " + str(ElapsedTime) + " estimated Expiry Time: " + str(self._percentileCancelTime))
            BelowCancelThreshold = EstimatedExpiryTimeOfFare > TimeToOrigin + FareExpiryLenience
            if not BelowCancelThreshold:
                print("will NOT bid on Fare as TimeToOrigin: " + str(TimeToOrigin) + " is greater than " + str(EstimatedExpiryTimeOfFare) + "\n")

        # Just goes for whatever is in range as is Desperate.
        if self._financialStatus == 0:
            BelowCancelThreshold = True
            PriceBetterThanCost = True

        NotCurrentlyBooked = NoCurrentPassengers and NoAllocatedFares
        Worthwhile = PriceBetterThanCost and NotCurrentlyBooked

        SufficientDrivingTime = FiniteTimeToOrigin and EnoughTimeToReachFare and BelowCancelThreshold
        CloseEnough = CanAffordToDrive and SufficientDrivingTime
...
class PsychoTaxi(Taxi):
    ...
    def __init__(self, world, taxi_num, idle_loss=256, max_wait=50, on_duty_time=0, off_duty_time=0, service_area=None,
        ...
        super().__init__(world, taxi_num, idle_loss * 2, max_wait, on_duty_time, off_duty_time, service_area,
                         start_point)
