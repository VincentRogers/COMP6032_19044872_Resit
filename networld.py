...
class NetWorld:
    ...
    def broadcastFare(self, origin, destination, price, percentileCancelTime, avgTaxisRevenue):
        ...
        for taxi in self._taxis.keys():
            if taxi.onDuty:
                onDuty += 1
                taxi.recvMsg(taxi.FARE_ADVICE, **{'origin': origin, 'destination': destination, 'price': price, 'percentileCancelTime': percentileCancelTime, 'avgTaxisRevenue': avgTaxisRevenue})
    ...
    # transmitFareBid is called by the taxi and notifies the Dispatcher that it is willing
